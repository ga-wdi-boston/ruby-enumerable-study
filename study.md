...
Redefining methods in array allows for more streamlined coding by having shortcut values for those methods. EX. {|a,e| a+e} vs (:+)
```
...
```md
Because the enumerable count function will only return elements in a block which can be defined as "true", rather than counting the number of spaces in the subject of the method.
```
...
```md
Enumerables are executed in one large chunk, whereas Streams can be executed piece by piece.
```
